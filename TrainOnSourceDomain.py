...
def Test(args, model, test_source_dataloader, test_target_dataloader, Best_Acc, epoch, confidence_target, writer, dist_target_test):
    """Test."""
    ...
    #@ m21-11-12, 只记录七个分类器的准确率
    accs = Show_OnlyAccuracy(acc)
    precs, recalls, f1s = Show_OtherMetrics(prec, recall, dist_target_test) # 返回值是七个分类器的prec, recall, F1
    ...
    'Accuracy: global {Accuracys[0]:.4f}\t left_eye {Accuracys[1]:.4f}\t right_eye {Accuracys[2]:.4f}\t nose {Accuracys[3]:.4f}\t left_mouse {Accuracys[4]:.4f}\t right_mouse {Accuracys[5]:.4f}\t global_local {Accuracys[6]:.4f}\n'\
    'Recall: global {Recalls[0]:.4f}\t left_eye {Recalls[1]:.4f}\t right_eye {Recalls[2]:.4f}\t nose {Recalls[3]:.4f}\t left_mouse {Recalls[4]:.4f}\t right_mouse {Recalls[5]:.4f}\t global_local {Recalls[6]:.4f}\n'\
    'Precision: global {Precisions[0]:.4f}\t left_eye {Precisions[1]:.4f}\t right_eye {Precisions[2]:.4f}\t nose {Precisions[3]:.4f}\t left_mouse {Precisions[4]:.4f}\t right_mouse {Precisions[5]:.4f}\t global_local {Precisions[6]:.4f}\n'\
    'F1 Score: global {F1[0]:.4f}\t left_eye {F1[1]:.4f}\t right_eye {F1[2]:.4f}\t nose {F1[3]:.4f}\t left_mouse {F1[4]:.4f}\t right_mouse {F1[5]:.4f}\t global_local {F1[6]:.4f}\n'\
    'Cls_Loss: global {Loss[0].avg:.4f}\t left_eye {Loss[1].avg:.4f}\t right_eye {Loss[2].avg:.4f}\t nose {Loss[3].avg:.4f}\t left_mouse {Loss[4].avg:.4f}\t right_mouse {Loss[5].avg:.4f}\t global_local {Loss[6].avg:.4f}\n'\
    'Situ_Acc: Situation_0 {six_acc[0].avg:.4f}\t Situation_1 {six_acc[1].avg:.4f}\t Situation_2 {six_acc[2].avg:.4f}\t Situation_3 {six_acc[3].avg:.4f}\t Situation_4 {six_acc[4].avg:.4f}\t Situation_5 {six_acc[5].avg:.4f}\n'\
    'Situ_Pro: Situation_0 {six_prob[0].avg:.4f}\t Situation_1 {six_prob[1].avg:.4f}\t Situation_2 {six_prob[2].avg:.4f}\t Situation_3 {six_prob[3].avg:.4f}\t Situation_4 {six_prob[4].avg:.4f}\t Situation_5 {six_prob[5].avg:.4f}\n\n'\
    .format(epoch, args.lr, Accuracys=accs, Recalls=recalls, Precisions=precs, F1=f1s, Loss=cls_loss, six_acc=six_accuracys, six_prob=six_probilities)
...
def main():
    ...
    train_source_dataloader, _, dist_source_train = BulidDataloader(args, flag1='train', flag2='source') # 构建了source的源域数据集生成器。调用他之后能够返回batch size数量的（裁剪后的人脸图片，人脸的五个关键点，表情标签）
    train_target_dataloader, init_train_dataset_data, dist_target_train = BulidDataloader(args, flag1='train', flag2='target') # 构建了训练的target目标域的数据集生成器，调用他之后能够返回batch size数量的（裁剪后的人脸图片，人脸的五个关键点，表情标签）
    test_source_dataloader, _, dist_source_test = BulidDataloader(args, flag1='test', flag2='source')   # test跟train的数据集生成器只有图像的预处理那里会有点不同
    test_target_dataloader, _, dist_target_test = BulidDataloader(args, flag1='test', flag2='target')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        Best_Acc = Test(args, model, test_source_dataloader, test_target_dataloader, Best_Acc, epoch, confidence, writer, dist_target_test)
